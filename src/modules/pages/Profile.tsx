...
import { queryClient } from '../context/layout-context';
...
export default function Profile() {
  ...
  const { data: userProfile, isLoading } = useFetchProfile(user?._id);
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      reset({
        birthDate: userProfile?.birthDate ? formatEntryDate(userProfile?.birthDate?.toString()) : '',
        location: {
          city: userProfile?.location?.city,
          country: userProfile?.location?.country,
        },
  ...
  const { mutate: updateImageMutation, isLoading: isLoadingImageUpdate } = useMutation({
  ...
  return (
    ...
    >
      ...
      <Box display="flex" alignItems="center" justifyContent="center" marginBottom="2rem" flexDirection="column">
        ...
        {!editPersonalInfo ? (
          <Box display="flex" flexDirection="column" justifyContent="center">
            ...
            {userProfile?.birthDate && (
              ...
              >
                {userProfile?.birthDate ? formatEntryDate(userProfile?.birthDate?.toString()) : ''}
              </Typography>
            )}
          ...
          <Grid container spacing={{ xs: 2, md: 2 }}>
            <Grid item xs={12} sm={12}>
              <Controller
                name="name"
                control={control}
                render={({ field }) => <TextField size="small" fullWidth label="Full Name" {...field} />}
              />
            </Grid>
            <Grid item xs={12} sm={12}>
              <Controller
            ...
            <Grid item xs={12} sm={6}>
              <Controller
                name="location.city"
                control={control}
                render={({ field }) => <TextField size="small" fullWidth label="City" {...field} />}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <Controller
                ...
                control={control}
                render={({ field }) => <TextField size="small" fullWidth label="Country" {...field} />}
              />
            </Grid>
            <Grid item xs={12} sm={12}>
              <Controller
                name="role"
                control={control}
                render={({ field }) => <TextField size="small" fullWidth label="Role" {...field} />}
              />
            </Grid>
          </Grid>
        )}
      ...
      {!editPersonalInfo ? (
        ...
        <RichText modules={modules} formats={formats} onChange={setBio} value={bio} theme="snow" />
      )}
